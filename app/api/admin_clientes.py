...
async def listar_medicos_cliente(
    ...
    """Lista todos os medicos/profissionais de um cliente"""
    try:
        medicos = db.execute(
            text("""
                SELECT id, nome, crm, especialidade, email, telefone,
                       ativo, pode_fazer_login, is_admin, is_secretaria,
                       email_verificado, criado_em
                FROM medicos
                WHERE cliente_id = :cliente_id
                ORDER BY is_admin DESC, is_secretaria ASC, nome ASC
            """),
            {"cliente_id": cliente_id}
        ).fetchall()
        ...
        return [
            {
                ...
                "telefone": m[5],
                "ativo": m[6],
                "pode_fazer_login": m[7],
                "is_admin": m[8],
                "is_secretaria": m[9],
                "email_verificado": m[10],
        ...
        logger.error(f"[Setup] Erro ao listar medicos: {e}", exc_info=True)
